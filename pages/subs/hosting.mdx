...
1. Create about 100 proxies following this [guide](/proxy/hosting) hosted anywhere.
2. Load the proxies into the `proxies.json` file in your Wyzie Subs repository.
3. Build Wyzie Subs and host it basically anywhere (I prefer Cloudflare Workers).
4. Ensure that a transform rule is configured to set `Access-Control-Allow-Origin` to `*`.